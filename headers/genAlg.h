...
/*
    ...
    1. Modify the ga_solution struct to save all posible solutions found for N-Queens(this implies modifying ga function to not end on the first solution found, but find all of them without saving repeated solutions.).
    2. Modify "CrossOver" function to work with N-Queens.(current implementation only works with 8-Queens).
    3. Implement more selection algorithms.
    4. Save an image of the solution representation. A chessboard(NxN) with the queens in their places. And maybe also implement it to save every individual from each generation, to see its evolution.
 */
...
/* 
    ...
    Params:
       ...
       double pCross:       Crossing probability used by the genetic algorithm to determine whether an individual should be crossedover with another one. Calculated as (1 - pMut).
...
/* 
    ...
    Params:
        ga_solution *sol:   A pointer to a ga_solution that was returned from the "ga" function.
*/
